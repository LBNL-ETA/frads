...
??? note "Default setting"
    ...
    **num_processors**: The number of processors to use for the simulation. (default=4)
    ...
    **sky_basis**: The sky basis to use for the simulation. (default="r4")
    ...
    **time_zone**: The timezone for the simulation. (default=120)
...
!!! tips "Use an EnergyPlus model to set up a workflow configuration"
    You can use the `epmodel_to_radmodel()` function to convert an EnergyPlus model to a Radiance model. The function returns a dictionary of the Radiance model for each exterior zone in the EnergyPlus model. You can use the dictionary to set up the workflow configuration.

    ```python
    epmodel = fr.load_energyplus_model("file.idf") # EnergyPlus model
    radmodel = fr.epmodel_to_radmodel(epmodel) # Radiance model
