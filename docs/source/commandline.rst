...
The configuration file (run.cfg) is generated using the following command from the project root directory.

.. code:: bash

   $ mrad init

The init command creates the Matrices, Resources, and Results directories to store the generated matrices, weather, and results from the simulation. It also creates the configuration file which consists of five sections: simulation control, file structure, site, model, and ray senders. 
