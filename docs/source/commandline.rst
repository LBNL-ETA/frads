...
mrad
----
A user can complete an entire matrix-based simulation using the mrad program, which takes a configuration file as input detailing the model setup, including the material and geometry data as Radiance primitives and model-specific information such as which geometry data corresponds to windows and walls.
...
A run.cfg file will be automatically generated using the Radiance model files present in the default directory structure, which includes an Objects directory for all model objects (e.g., floor, walls, ceiling) with an .rad extension and material definition with an .mat file extension.  The init command will then make Matrices, Resources, and Results directories to store the generated matrices, weather, and results from the simulation. The init command also provides a series of default simulation parameters for the various matrix generation procedures. A typical configuration file consists of five sections: simulation control, file structure, site, model, and ray senders. An excerpt of the automatically generated configuration looks like this::
...
The file structure section of the configuration need not be changed unless the user elects to define a different one. The structure defaults to::
...
Ray sender represents the objects used to spawn rays: grid points and view (image rendering). A user has to define at least one ray sender for mrad to carry out the simulation. For grid-based illuminance or irradiance simulations (e.g., workplane illuminance), the grid surface, usually floor, must be included::
...
If a configuration file has been prepared beforehand, for example as test.cfg, simply append the file name as such::
...
Put the following simple geometry in Radiance format in a local **Objects** directory::
...
After the simulation has finished, the results are in the Results directory. In the case of workplane illuminance results::
