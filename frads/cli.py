...
def glazing():
    ...
    aparser.add_argument('-X', '--optics', nargs='+',
                         type=Path, help='Optics file path')
...
def genmtx_sun(sender: Sender, env, args: argparse.Namespace) -> None:
    ...
    receiver = matrix.sun_as_receiver(
        ...
        window_normals=wnormals, full_mod=full_modifier)
    ...
    with tf.TemporaryDirectory(dir=os.getcwd()) as tempd:
        mod_names = ["%04d" % (int(line[3:])-1)
                     for line in receiver.modifier.splitlines()]
...
def genmtx_surface(sender: matrix.Sender, env, args: argparse.Namespace) -> None:
    ...
    matrix.rfluxmtx(sender=sender, receiver=receiver, env=env, opt=args.option, out=None)
...
def genmtx():
    ...
    if args.surface:
        prims = utils.unpack_primitives(args.surface)
        sender = matrix.surface_as_sender(prim_list=prims,
                                      basis=args.sbasis,
                                      offset=args.soffset)
        args.surface.close()
    elif args.view:
        vudict = parsers.parse_vu(args.view.readlines()[-1])  # use the last view
        sender = matrix.view_as_sender(vu_dict=vudict, ray_cnt=args.ray_count,
...
def rpxop():
    ...
    parser_dcts.add_argument('-n', type=int,
                             help='number of processors to use')
    parser_pcomb = subparser.add_parser("pcomb")
    parser_pcomb.set_defaults(func=mtxmult.batch_pcomb)
    parser_pcomb.add_argument('inp', type=str, nargs="+",
                              help='list of inputs, e.g., inp1 + inp2.hdr')
