...
def get_wea(config: ConfigParser, window_normals=None):
    """Obtain and prepare weather file data."""
    remove_zero = True if window_normals is not None else False
    if (wea_path := config["Site"]["wea_path"]) != "":
        logger.info('Using user specified %s file.' % wea_path)
        with open(wea_path) as rdr:
            raw = rdr.read()
        wea_metadata, wea_data = makesky.parse_wea(raw)
        if window_normals is not None:
            wea_path = util.basename(wea_path) + '_d6.wea'
        ...
        if window_normals is None:
            wea_path = util.basename(epw_path) + '.wea'
        else:
            wea_path = util.basename(epw_path) + '_d6.wea'
    if "" not in (config["Site"]["start_hour"], config["Site"]["end_hour"]):
        wea_data = makesky.start_end_hour(
            wea_data,
            float(config["Site"]["start_hour"]),
            float(config["Site"]["end_hour"]))
    if config.getboolean("Site", "daylight_hours_only", fallback=False):
...
def get_window_group(config: ConfigParser) -> Tuple[dict, list]:
    ...
    for wpath in config["Model"]["window_paths"].split():
        wname = util.basename(wpath)
        _window_primitives = radutil.unpack_primitives(wpath)
...
def get_sender_grid(config: ConfigParser) -> dict:
    """."""
    sender_grid = {}
    for gpath in config["RaySender"]["grid_surface"].split():
        name = util.basename(gpath)
        surface_polygon = radutil.parse_polygon(
            radutil.unpack_primitives(gpath)[0].real_arg)
...
def get_sender_view(config: ConfigParser) -> Tuple[dict, dict]:
    ...
    sender_view[view_name] = radmtx.Sender.as_view(
        vu_dict=vdict, ray_cnt=int(config["SimControl"]["ray_count"]),
        xres=vdict['x'], yres=vdict['y'])
    return sender_view, view_dicts


def assemble_model(config: ConfigParser) -> Model:
    """Assemble all the pieces together."""
    material_primitives: List[radutil.Primitive]
    material_primitives = sum([radutil.unpack_primitives(path)
    ...
    rcvr_sky = radmtx.Receiver.as_sky(
        basis=config["SimControl"]["smx_basis"])
    cfs_path = []
    if "window_cfs" in config["Model"]:
        cfs_path = config["Model"]["window_cfs"].split()
    bsdf_mat = {wname: path for wname, path in
                zip(window_groups, config["Model"]["window_xml"].split())}
    black_mat = radutil.Primitive(
        'void', 'plastic', 'black', '0', '5 0 0 0 0 0')
    ...
    material_path = "all_material.mat"
    ...
    return Model(material_path, window_groups, window_normals,
                 ...
                 bsdf_mat, cfs_path, _blackenvpath)


def prep_2phase_pt(model: Model, config: ConfigParser):
    ...
    for grid_name, sender_grid in model.sender_grid.items():
        pdsmx[grid_name] = os.path.join("Matrices", f"pdsmx_{grid_name}.mtx")
        if ((not os.path.isfile(pdsmx[grid_name])) or
            config.getboolean("SimControl", "overwrite", fallback=False)):
...
def prep_2phase_vu(model: Model, config: ConfigParser):
    ...
    for view_name, sender_view in model.sender_view.items():
        vdsmx[view_name] = os.path.join("Matrices", f"vdsmx_{view_name}")
        if ((not os.path.isdir(vdsmx[view_name])) or
            config.getboolean("SimControl", "overwrite", fallback=False)):
            logger.info("Generating for %s" % view_name)
            radmtx.rfluxmtx(sender=sender_view, receiver=model.receiver_sky,
                            env=env, opt=opt, out=vdsmx[view_name])
    return vdsmx


def view_matrix_pt(model: Model, config: ConfigParser, direct=False):
    """."""
    ...
    receiver_windows = radmtx.Receiver(
        receiver='', basis=config["SimControl"]["vmx_basis"], modifier=None)
    for grid_name, sender_grid in model.sender_grid.items():
        for wname, window_prim in model.window_groups.items():
            ...
            vmxs[grid_name + wname] = os.path.join(
                "Matrices", f'pvmx_{_name}.mtx')
            receiver_windows += radmtx.Receiver.as_surface(
                prim_list=window_prim, basis=config["SimControl"]["vmx_basis"],
...
def view_matrix_vu(model, config: ConfigParser, direct=False):
    ...
    for view in model.sender_view:
        ...
        for wname, window_prim in model.window_groups.items():
            ...
            vmxs[view+wname] = os.path.join(
                "Matrices", f'vvmx_{_name}', '%04d.hdr')
    ...
    for view, sender_view in model.sender_view.items():
        for wname in model.window_groups:
            if ((not os.path.isdir(vmxs[view+wname][:-8])) or
                    config.getboolean(
                        "SimControl", "overwrite", fallback=False)):
...
def facade_matrix(model: Model, config: ConfigParser, direct=False):
    ...
    fmxs = {}
    _opt = config["SimControl"]["dmx_opt"]
    _env = [model.material_path] + config["Model"]["scene"].split()
    if direct:
        _opt += ' -ab 0'
        _env = [model.material_path, model.blackenvpath]
    ...
    for wname in model.window_groups:
        ...
        fmxs[wname] = os.path.join("Matrices", f'fmx_{_name}.mtx')
        ...
        if ((not os.path.isfile(fmxs[wname])) or
                config.getboolean(
                    "SimControl", "overwrite", fallback=False)):
...
def daylight_matrix(sender_prims, model: Model,
        config: ConfigParser, direct=False):
    ...
    for sname, surface_primitives in sender_prims.items():
        ...
        if direct:
            _name += '_d'
        dmxs[sname] = os.path.join("Matrices", f'dmx_{_name}.mtx')
        sndr_window = radmtx.Sender.as_surface(
            prim_list=surface_primitives,
            basis=config["SimControl"]["vmx_basis"])
        if ((not os.path.isfile(dmxs[sname])) or
                config.getboolean(
                    "SimControl", "overwrite", fallback=False)):
...
def blacken_env(model, config: ConfigParser):
    ...
    vmap_oct = 'vmap.oct'
    cdmap_oct = 'cdmap.oct'
    vmap = util.spcheckout(['oconv', model.material_path]+config["Model"]["scene"].split()+[bwindow_path])
    with open(vmap_oct, 'wb') as wtr:
        wtr.write(vmap)
    cdmap = util.spcheckout(['oconv', '-f', model.material_path]+config["Model"]["scene"].split()+[gwindow_path])
    with open(cdmap_oct, 'wb') as wtr:
        wtr.write(cdmap)
    return vmap_oct, cdmap_oct


def calc_4phase_pt(model, datetime_stamps, vmx, fmx, dmx, smx, config: ConfigParser):
    """."""
...
def prep_4phase_pt(model, config: ConfigParser, direct=False):
    ...
    for wname in model.window_groups:
        ...
        if direct:
            _name = wname + '_d'
        fmxs[wname] = os.path.join("Matrices", 'fmx_{_name}.mtx')
        ...
        mfacade.Genfmtx(win_polygons=window_prim['polygon'], port_prim=port_prims,
                        out=fmxs[wname], env=_env, sbasis=config["SimControl"]["vmx_basis"],
                        rbasis=config["SimControl"]["fmx_basis"], opt=_opt, refl=False,
                        forw=False, wrap=False)
        logger.info(f"Generating daylight matrix for {wname}")
        dmxs[wname] = os.path.join("Matrices", f'dmx_{wname}.mtx')
        ...
        radmtx.rfluxmtx(sender=sndr_port, receiver=model.receiver_sky,
                        env=_env, out=dmxs[wname], opt=config["SimControl"]["dmx_opt"])
    return fmxs, dmxs


def direct_sun_matrix_pt(model, smx_path, config: ConfigParser):
    ...
    for grid_name, sender_grid in model.sender_grid.items():
        pcdsmx[grid_name] = os.path.join(
            "Matrices", f'pcdsmx_{grid_name}.mtx')
        if ((not os.path.isfile(pcdsmx[grid_name])) or
                config.getboolean(
                    "SimControl", "overwrite", fallback=False)):
            ...
            radmtx.rcontrib(sender=sender_grid, modifier=rcvr_sun.modifier,
                            ...
                            opt=config["SimControl"]["cdsmx_opt"])
    return pcdsmx


def direct_sun_matrix_vu(model, smx_path, vmap_oct, cdmap_oct, config: ConfigParser):
    ...
    for view, sndr in model.sender_view.items():
        vmap_paths[view] = os.path.join("Matrices", f'vmap_{view}.hdr')
        cdmap_paths[view] = os.path.join("Matrices", f'cdmap_{view}.hdr')
        ...
        with open(cdmap_paths[view], 'wb') as wtr:
            wtr.write(cdmap)
        vcdfmx[view] = os.path.join("Matrices", f'vcdfmx_{view}')
        vcdrmx[view] = os.path.join("Matrices", f'vcdrmx_{view}')
        tempf = os.path.join("Matrices", 'vcdfmx')
        tempr = os.path.join("Matrices", 'vcdrmx')
        if ((not os.path.isdir(vcdfmx[view])) or
                config.getboolean(
                    "SimControl", "overwrite", fallback=False)):
            logger.info(
            ...
            radmtx.rcontrib(sender=sndr, modifier=rcvr_sun.modifier,
                            ...
                            opt=config["SimControl"]["cdsmx_opt"] + ' -n %s' % config["SimControl"]["nprocess"])
            ...
            shutil.rmtree(tempf)
        if ((not os.path.isdir(vcdrmx[view])) or
                config.getboolean(
                    "SimControl", "overwrite", fallback=False)):
            logger.info(
            ...
            radmtx.rcontrib(sender=sndr, modifier=rcvr_sun.modifier,
                            ...
                            opt=config["SimControl"]["cdsmx_opt"] + ' -i -n %s' % config["SimControl"]["nprocess"])
...
def calc_2phase_pt(model, datetime_stamps, dsmx, smx, config: ConfigParser):
    """."""
    ...
    for grid_name in dsmx:
        ...
        opath = os.path.join(
            "Results", f'grid_{config["Model"]["name"]}_{grid_name}.txt')
...
def calc_2phase_vu(datetime_stamps, dsmx, smx, config: ConfigParser):
    """."""
    logger.info("Computing for 2-phase image-based results")
    for view in dsmx:
        opath = os.path.join(
                "Results", f'view_{config["Model"]["name"]}_{view}')
...
def calc_3phase_pt(model, datetime_stamps, vmx, dmx, smx, config: ConfigParser):
    """."""
    logger.info("Computing for 3-phase sensor grid results")
    for grid_name in model.sender_grid:
        presl = []
        for wname in model.window_groups:
            _res = mtxmult.mtxmult(vmx[grid_name+wname], model.bsdf_xml[wname],
                           dmx[wname], smx)
...
def calc_3phase_vu(model, datetime_stamps, vmx, dmx, smx, config: ConfigParser):
    """."""
    ...
    for view in model.sender_view:
        ...
        vresl = []
        for wname in model.window_groups:
            _vrespath = os.path.join("Results", f'{view}_{wname}')
            util.mkdir_p(_vrespath)
            cmd = mtxmult.imgmult(vmx[view+wname], model.bsdf_xml[wname],
...
def calc_5phase_pt(model, vmx, vmxd, dmx, dmxd, pcdsmx,
        smx, smxd, smx_sun, datetime_stamps, config: ConfigParser):
    """."""
    ...
    for grid_name in model.sender_grid:
        ...
        for wname in model.window_groups:
            _res = mtxmult.mtxmult(vmx[grid_name+wname], model.bsdf_xml[wname], dmx[wname], smx)
            _resd = mtxmult.mtxmult(
                vmxd[grid_name+wname], model.bsdf_xml[wname], dmxd[wname], smxd)
            if isinstance(_res, bytes):
        ...
        respath = os.path.join("Results", f'grid_{config["Model"]["name"]}_{grid_name}.txt')
        with open(respath, 'w') as wtr:
...
def calc_5phase_vu(model, vmx, vmxd, dmx, dmxd, vcdrmx, vcdfmx,
                   ...
                   datetime_stamps, datetime_stamps_d6, config: ConfigParser):
    """Compute for image-based 5-phase method result."""
    nprocess = int(config["SimControl"]["nprocess"])
    for view in model.sender_view:
        ...
        with tf.TemporaryDirectory() as td:
            ...
            for wname in model.window_groups:
                ...
                cmds.append(mtxmult.imgmult(vmx[view+wname], model.bsdf_xml[wname],
                                    dmx[wname], smx, odir=_vrespath))
                cmds.append(mtxmult.imgmult(vmxd[view+wname], model.bsdf_xml[wname],
                                    dmxd[wname], smxd, odir=_vdrespath))
                vresl.append(_vrespath)
                vdresl.append(_vdrespath)
            logger.info("Multiplying matrices for images.")
            process = mp.Pool(nprocess)
            ...
            if len(model.window_groups) > 1:
                for i in range(1, len(vresl)):
                    vresl.insert(i * 2 - 1, '+')
                for i in range(1, len(vdresl)):
                    vdresl.insert(i * 2 - 1, '+')
                mtxmult.pcombop(
                    vresl, res3, nproc=nprocess)
                mtxmult.pcombop(
                    vdresl, res3di, nproc=nprocess)
            else:
...
def three_phase(model, config: ConfigParser, direct=False):
    ...
    if direct:
        ...
        smxd = gen_smx(wea_path, config["SimControl"]["smx_basis"], "Matrices", direct=True)
        smx_sun_img = gen_smx(wea_path_d6, config["SimControl"]["cdsmx_basis"],
                              "Matrices", onesun=True, direct=True)
        smx_sun = gen_smx(wea_path, config["SimControl"]["cdsmx_basis"], "Matrices",
        ...
        os.remove(vmap_oct)
        os.remove(cdmap_oct)
        os.remove("glowing_window.rad")
        os.remove("blackened_window.rad")
        os.remove("sun.oct")
        os.remove(wea_path_d6)
        os.remove(model.blackenvpath)
        if not config.getboolean(
                "SimControl", "no_multiply", fallback=False):
...
def four_phase(model, config: ConfigParser, direct=False):
    ...
    if direct:
        ...
        calc_4phase_pt(model, datetime_stamps, pvmxs, fmxs, dmxs, smx, config)
